...
describe("tests for the trash label", () => {
  before(async () => {
  ...
  test("0. Setup: Register, login, create label and mail", async () => {
    ...
    nonAttachableLabels = labels.filter(
      (label) => label.isAttachable === false,
    );
  ...
  test("Remove trash label from mail", async () => {
    await api
      .delete(`/api/mails/${mailId}/labels/${trashLabelId}`)
      .set("Authorization", "bearer " + token)
      .expect(200);
    ...
    assert(
      !mail.labels.includes(trashLabelId),
      "Mail should not have trash label",
    );
  });
  ...
  test("List inbox should include the mail again", async () => {
    ...
    assert(
      mails.some((mail) => mail.id === mailId),
      "Inbox should include the mail again",
    );
  });
  ...
  test("List mails by trash label should not include the mail", async () => {
    ...
    assert(
      !mails.some((mail) => mail.id === mailId),
      "trash label should not include the mail",
    );
  });
